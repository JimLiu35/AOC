...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] ) 
 { 
    ...
    acadodata_f2 << u_delta;
    DVector acadodata_v1(1);
    acadodata_v1(0) = 0;
    DVector acadodata_v2(1);
    acadodata_v2(0) = 0;
    DMatrix acadodata_M1;
    acadodata_M1.read( "hw8_car_time_optimal_data_acadodata_M1.txt" );
    DifferentialEquation acadodata_f1(0, T);
    acadodata_f1 << dot(Px) == cos(theta)*v;
    acadodata_f1 << dot(Py) == sin(theta)*v;
    acadodata_f1 << dot(theta) == tan(delta)*v;
    acadodata_f1 << dot(v) == ua;
    acadodata_f1 << dot(delta) == u_delta;

    OCP ocp1(0, T, 64);
    ocp1.minimizeMayerTerm(T);
    ocp1.minimizeLSQ(acadodata_f2, acadodata_v2);
    ocp1.subjectTo(acadodata_f1);
    ocp1.subjectTo((-5.00000000000000000000e+00) <= v <= 5.00000000000000000000e+00);
    ...
    ocp1.subjectTo((-5.23598775598298815659e-01) <= u_delta <= 5.23598775598298815659e-01);
    ocp1.subjectTo((1.00000000000000000000e+00-pow((7.00000000000000000000e+00+Px),2.00000000000000000000e+00)-pow(Py,2.00000000000000000000e+00)) <= 0.00000000000000000000e+00);
    ocp1.subjectTo(3.00000000000000000000e+00 <= T <= 1.00000000000000000000e+01);
    ocp1.subjectTo(AT_START, Px == (-1.00000000000000000000e+01));
    ocp1.subjectTo(AT_START, Py == 1.00000000000000000000e+00);
    ...
    ocp1.subjectTo(AT_END, Px == 0.00000000000000000000e+00);
    ocp1.subjectTo(AT_END, Py == 0.00000000000000000000e+00);
    ...
    algo1.initializeControls( acadodata_M1 );
